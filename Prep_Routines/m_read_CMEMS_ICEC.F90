module m_read_CMEMS_ICEC

contains

  subroutine read_CMEMS_ICEC_repro(fname, data, gr)
    use nfw_mod
    use mod_measurement
    use mod_grid
    implicit none
...
!          data(nobs) % var = max(1d-8 * std(i, j) ** 2, 0.01d0 + (0.5d0 - abs(0.5d0 - data(nobs) % d)) ** 2)
  ...
  end subroutine read_CMEMS_ICEC_repro
  ...
  subroutine read_CMEMS_ICEC_norepro(fname, data, gr)
    use nfw_mod
    use mod_measurement
    use mod_grid
    implicit none
...
!          data(nobs) % var =  0.01d0 + (0.5d0 - abs(0.5d0 - data(nobs) % d)) ** 2
  ...
  end subroutine read_CMEMS_ICEC_norepro

end module m_read_CMEMS_ICEC
