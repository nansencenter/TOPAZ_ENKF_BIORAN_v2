...
!                NERSC
!
! Purpose:       Read T/S profile data from NetCDF files from CMEMS CORA into TOPAZ
!                system.
!
...
!                00/03/2024 JX: skip *_qc=2 in the profile4
!                14/03/2025 TW: change time variable from JULD to TIME
!                               read QC data as byte type: integer(kind=1)
...
contains
  ...
  subroutine read_CMEMS_CORA(fnames, obstype, variance, nx, ny, data,datainfo)
    use mod_measurement
    use m_oldtonew
    use m_confmap
    use m_bilincoeff
    ...
    integer(kind=1), allocatable :: time_qc(:)
    real(8), allocatable :: lat(:), lon(:)
    integer(kind=1), allocatable :: pos_qc(:)
    real(8), allocatable :: pres(:,:)
    integer(kind=1), allocatable :: pres_qc(:,:)
    ...
    print *, 'BEGIN read_CMEMS_CORA(): ',trim(fnames)
    ...
    do f = 1, nfile
#if defined (BLACK_PROFILE)
       call data_readfile(f, trim(obstype), np, time(p : nprof),&
            time_qc(p : nprof), lat(p : nprof),&
            ...
            Pdatainfo(p:p+np-1)%jday=floor(time(p:nprof));
            Pdatainfo(p:p+np-1)%jtime=time(p:nprof)-floor(time(p:nprof));
    ...
    ! mask <- time_qc, pos_qc, pres_qc, v_qc
    !
    allocate(mask(nprof))
    mask(:) = 1
...
#endif
    where (time_qc /= 1) mask = 0
    !where (time_qc /= 1 .and. time_qc /= 2) mask = 0
    do p = 1, nprof
       if (mask(p) == 0) then
          mask2(:, p) = 0
       end if
    ...
    do p = 1, nprof
       ...
       ! output the profile list within the model domain
          if (mask_0(p)==0) then
            !write(20,'(a16,F13.5,2F10.4,2x,a)') trim(Profname(p)),time(p),lon(p),lat(p),trim(Pfsur(Ptrack(p)))  
       ...
       do l = 1, nlev
          ...
          if (ndata > ngood) then
             print *, 'ERROR: read_CMEMS_CORA(): programming error'
             print *, '  p =', p, ', l =', l
...
! saving the related measurment information
  ...
  subroutine data_readfile(fid, obstype, nprof, time_all, time_qc_all,&
    ...
    real(8), intent(inout), dimension(:) :: time_all
    integer(kind=1), intent(inout), dimension(:) :: time_qc_all
    real(8), intent(inout), dimension(:) :: lat_all, lon_all
    integer(kind=1), intent(inout), dimension(:) :: pos_qc_all
    real(8), intent(inout), dimension(:,:) :: pres_all
    integer(kind=1), intent(inout), dimension(:,:) :: pres_qc_all
    real(8), intent(inout), dimension(:,:) :: temp_all
    integer(kind=1), intent(inout), dimension(:,:) :: temp_qc_all
    real(8), intent(inout), dimension(:,:) :: salt_all
    integer(kind=1), intent(inout), dimension(:,:) :: salt_qc_all
   ...
   ! integer, external :: strrindex
    ...
    call nfw_get_var_double(fname, ncid, id, time_all(1 : nprof))

    ! time_qc
    !
    call nfw_inq_varid(fname, ncid, 'TIME_QC', id)
    call nfw_get_var_int1(fname, ncid, id, time_qc_all(1 : nprof))
    ...
    call nfw_get_var_int1(fname, ncid, id, pos_qc_all(1 : nprof))
    ...
    if (nfw_var_exists(ncid, 'PRES')) then
      ...
      call nfw_get_var_int1(fname, ncid, id, pres_qc_all(1 : nlev, 1 : nprof))
    else
      ! pres
      !
    ...
    if (nfw_var_exists(ncid, 'TEMP')) then
      ...
      call nfw_get_var_int1(fname, ncid, id, temp_qc_all(1 : nlev, 1 : nprof))
    else
       temp_qc_all = 'E';
    endif
    ...
    if (nfw_var_exists(ncid, 'PSAL')) then
      ...
      if (varmin(1)<varmax(1).and.varmax(1)>0) then
       ...
       call nfw_get_var_int1(fname, ncid, id, salt_qc_all(1 : nlev, 1 : nprof))
    else
       salt_qc_all = 'E';
    end if
